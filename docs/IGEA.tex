...
% ------------------------------------------
    ...
    \paragraph{Naive Bayes} Il Naive Bayes è un classificatore probabilistico basato sull’applicazione del
    Teorema di Bayes, che assume l’indipendenza condizionata tra le variabili date le classi.

    Nel progetto è stata adottata una variante di tipo Bernoulli, particolarmente adatta a feature
    binari ottenuti tramite One-Hot Encoding. Tale scelta risulta coerente con la rappresentazione
    dei dati utilizzata nella pipeline di modellazione e con la natura delle variabili derivate da
    questionari.

    \begin{itemize}
        \item \textbf{Natura Probabilistica}: Il modello restituisce una stima esplicita della probabilità
        di appartenenza alla classe di interesse, permettendo una valutazione graduale del rischio.
        \item \textbf{Efficienza Computazionale}: Il numero ridotto di parametri da stimare rende il
        modello estremamente efficiente sia in fase di addestramento che di inferenza.
        \item \textbf{Approccio Generativo}: A differenza della regressione logistica, il Naive Bayes
        modella la distribuzione congiunta $P(x, y)$, offrendo una prospettiva complementare e un
        utile termine di confronto per la validazione dei risultati.
    \end{itemize}
