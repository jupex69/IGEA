...
% ==========================================
    ...
    Le due pipeline rappresentano approcci complementari alla preparazione del dataset. La prima adotta una strategia
    più conservativa, basata sulla rimozione delle variabili potenzialmente ridondanti, poco informative o fortemente
    dipendenti dal contesto di origine del dataset. Questa pipeline consente di valutare le prestazioni dei modelli
    in una configurazione minimale, riducendo il rischio di bias e di dipendenza da specifiche assunzioni sul dominio.

    La seconda pipeline, invece, mantiene alcune variabili di contesto accademico, introducendo trasformazioni
    controllate volte a migliorarne l’interpretabilità e la trasferibilità nel contesto universitario di riferimento.
    In particolare, tale pipeline consente di analizzare l’effetto di una rappresentazione più informata dei dati, pur
    senza introdurre assunzioni forti o modifiche che alterino il significato originale delle informazioni.
    ...
    \begin{itemize}
        ...
        \item \textit{Have you ever had suicidal thoughts ?}: variabile fortemente correlata al target che potrebbe
        introdurre un fenomeno di \textit{data leakage}, fornendo al modello informazioni non realisticamente
        disponibili in fase di predizione.
    \end{itemize}
    ...
    In questa fase di preparazione dei dati si è scelto di mantenere le variabili numeriche nelle loro scale originali.
    Tale scelta è motivata dalle seguenti considerazion:

    \begin{itemize}
        \item \textbf{Conservazione del significato fisico}: l'utilizzo dei valori non normalizzati consente di preservare
        il significato fisico delle variabili, facilitando sia l'analisi esplorativa dei dati sia l’interpretazione
        dei risultati del modello, in quanto l’effetto delle caratteristiche sul target può essere valutato
        direttamente nelle rispettive unità di misura.
        \item \textbf{Coerenza con i modelli adottati}: le pipeline di progetto prevedono l'utilizzo di algoritmi
        \textit{classificazione} che non basano il proprio funzionamento su metriche di distanza
        o combinazioni lineari delle caratteristiche.
    \end{itemize}
