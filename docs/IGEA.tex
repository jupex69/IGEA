...
% ==========================================
    ...
    È stata eliminata la feature \textit{Have you ever had suicidal thoughts ?} considerata la natura semantica della feature,
    ...
    Inizialmente sono state rimosse le feature non informative o dipendenti dal contesto di origine del dataset:
    \begin{itemize}
    ...
    \subsection{Pipeline 2}
        ...
        La pipeline 2 mantiene le info accademiche ma le rende più trasferibili/leggibili (Degree aggregato, CGPA in 30),
        evitando variabili non trasferibili (id/city) o ridondanti (Work Pressure, Job Satisfaction, Profession).
    ...
    \subsubsection{Feature engineering}
        ...
        \paragraph{Feature Transformation CGPA}
            ...
            La variabile \textit{CGPA}, originariamente espressa su scala decimale (dove, secondo le specifiche del dataset, un valore di 4.0 rappresenta il massimo punteggio  e 10.0 il minimo),
            è stata convertita in una rappresentazione su scala trentesimale,
            al fine di rendere i valori interpretabili nel contesto universitario italiano.
            La conversione è stata effettuata mediante una trasformazione lineare inversa, mappando i punti noti della scala (4.0 \to 30 e 10.0 \to 18).
            La formulaapplicata è la seguente:

            \[ CGPA_{30} = -2 \cdot CGPA + 38 \]

            Tale trasformazione preserva le proprietà statistiche della variabile e la sua capacità predittiva,
            ma ha esclusivamente una finalità descrittiva e interpretativa.

            Nel dataset finale, la variabile originale \textit{CGPA} è stata sostituita dalla nuova rappresentazione \textit{CGPA\_30},
            al fine di evitare ridondanze informative.

        \paragraph{Feature Selection}
            Analogamente alla pipeline 1 sono state rimosse le feature:
            \begin{itemize}
                \item \textbf {id}: identificativo univoco irrilevante per la predizione del rischio di depressione;
                \item \textbf {city}: fortemente dipendente dal contesto geografico indiano e quindi non generalizzabile ad altri contesti universitari;
            \end{itemize}
...
% ------------------------------------------
    ...
    statistici differenti: la \textbf{Logistic Regression} e il \textbf{Naive Bayes}.
    La scelta è motivata dalla necessità di bilanciare accuratezza predittiva e trasparenza nell'analisi
    ...
    Il Naive Bayes è un classificatore probabilistico basato sull'applicazione del Teorema di Bayes
    con l'assunzione di indipendenza tra le variabili.
    \begin{itemize}
        \item \textbf{Modellazione delle Variabili Continue:} L'algoritmo assume che le feature
        numeriche seguano una distribuzione normale (Gaussiana). Tale assunzione è coerente con la fase di \textit{Standardization}
        applicata al dataset, che trasforma le variabili per avere media nulla e varianza unitaria.
        \item \textbf{Efficienza Computazionale:} Il modello richiede un numero ridotto di parametri da stimare,
        risultando estremamente rapido sia in fase di addestramento che di inferenza,
        pur mantenendo buone capacità di generalizzazione.
        \item \textbf{Approccio Generativo:} A differenza della regressione logistica (modello discriminativo),
        il Naive Bayes offre una prospettiva diversa modellando la distribuzione congiunta $P(x,y)$,
        fornendo un utile termine di confronto per la validazione dei risultati.
    \end{itemize}

    \subsection{Train-Test Split}

    Per valutare correttamente la capacità di generalizzazione dei modelli e prevenire il fenomeno dell'
    \textit{overfitting}, il dataset è stato suddiviso secondo la metodologia del \textbf{Hold-out},
    utilizzando una proporzione \textbf{80/20}.

    Questa tecnica prevede la creazione di due sottoinsiemi indipendenti:
    \begin{itemize}
        \item \textbf{Training Set (80\%):} utilizzato per l'addestramento.
        \item \textbf{Test Set (20\%):} permette di misurare le prestazioni su dati non visti,
            assicurando che il modello non abbia semplicemente 'imparato a memoria' il dataset di
            addestramento

    \end{itemize}

    \paragraph{Riproducibilità}
        Al fine di garantire la riproducibilità dei risultati, la suddivisione è stata eseguita utilizzando
        un seme di generazione casuale fissato (\textit{random\_state}).
        Ciò permette di ottenere la stessa partizione di dati in ogni esecuzione del codice,
        facilitando il confronto coerente tra i diversi modelli testati.
