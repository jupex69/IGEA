...
\begin{document}
    ...
    \begin{titlepage}
        ...
        % Link al repository
        \large
        \textbf{Link al repository:} \href{https://github.com/jupex69/IGEA}{https://github.com/jupex69/IGEA} \\[0.8cm]
...
% ==========================================
    ...
    \subsection{Feature Scaling}
        Per garantire che tutte le feature abbiano la stessa importanza, si è deciso (per entrambe le Pipeline) di normalizzare
    ...
    \begin{table}[h!]
        \centering
        \caption{Confronto delle feature sottoposte a Scaling nelle due Pipeline}
        \label{tab:feature_scaling_comparison}
        \renewcommand{\arraystretch}{1.3} % Aumenta leggermente lo spazio tra le righe
        \begin{tabular}{|l|l|}
            \hline
            \textbf{Pipeline 1} & \textbf{Pipeline 2} \\
            \hline
            \textit{Age} & \textit{Age} \\
            \textit{Study Satisfaction} & \textit{Study Satisfaction} \\
            \textit{Work/Study Hours} & \textit{Work/Study Hours} \\
            \textit{Stress\_Amplified} & \textit{Academic Pressure} \\
            & \textit{CGPA\_30} \\
            \hline
        \end{tabular}
    \end{table}
    ...
    \newpage
    \subsection{Encoding}
        Poiché la maggior parte dei modelli di machine learning opera su numeri si è deciso (per entrambe le Pipeline) di convertire
    ...
    \begin{table}[h!]
        \centering
        \caption{Confronto delle feature sottoposte a Encoding nelle due Pipeline}
        \label{tab:feature_encoding_comparison}
        \renewcommand{\arraystretch}{1.3} % Spaziatura migliore
        \begin{tabular}{|l|l|}
            \hline
            \textbf{Pipeline 1} & \textbf{Pipeline 2} \\
            \hline
            \textit{Gender} & \textit{Gender} \\
            \textit{Dietary Habits} & \textit{Dietary Habits} \\
            \textit{Family History of Mental Illness} & \textit{Family History of Mental Illness} \\
            & \textit{Sleep Duration} \\
            & \textit{Financial Stress} \\
            & \textit{Degree\_level} \\
            \hline
        \end{tabular}
...
% ------------------------------------------
    ...
    Nel caso del \textbf{Decision Tree}, l’addestramento è stato preceduto da una fase di
    \textbf{ottimizzazione degli iperparametri} mediante \textit{Grid Search}, integrata nel processo di
    cross-validation. Sono state esplorate diverse configurazioni relative alla profondità massima
    dell’albero, al numero minimo di campioni richiesti per effettuare uno split e per la creazione delle
    foglie, nonché al criterio di impurità adottato.
    La configurazione ottimale è stata selezionata sulla base delle prestazioni medie ottenute sui fold di
    validazione.
    ...
    Al termine dell’addestramento, ciascun modello è stato valutato tramite cross-validation utilizzando
    un insieme di metriche complementari, includendo accuracy, precision, recall, F1-score e ROC AUC, al
    fine di fornire una valutazione completa delle prestazioni predittive e dei diversi tipi di errore.


% ------------------------------------------

    \section{Conclusioni}
    L'analisi condotta attraverso le due pipeline di preparation e i due algoritmi di classificazione
    ha permesso di identificare i fattori più
    influenti nella previsione della depressione studentesca e di selezionare il modello più performante.
    ...
    \textbf{Pipeline 2 + Logistic Regression} offre le prestazioni migliori in assoluto,
    raggiungendo una \textit{Precision} del \textbf{84.43\%} e una \textit{Recall} di \textbf{79.43\%}.

    \includegraphics[width=1.1\textwidth]{images/graphs/graficoPrestazioni} \\[0.4cm]

    \subsection{La nostra scelta: Logistic Regression (Pipeline 2)}

    Alla luce dei risultati ottenuti, il modello selezionato per la fase di deployment è la \textbf{Logistic Regression}
    addestrata sui dati processati dalla \textbf{Pipeline 2}.
    Questa combinazione ha garantito il miglior compromesso tra capacità di generalizzazione
    ...
    \begin{table}[h!]
        ...
        \begin{tabular}{|l|c|c|l|}
            ...
            \hline
            Financial Stress (Level 5) & +2.27 & 9.70 \\
            Financial Stress (Level 4) & +1.63 & 5.10 \\
            Academic Pressure & +1.16 & 3.19 \\
            Dietary Habits (Unhealthy) & +1.05 & 2.85 \\
            Age & -0.54 & 0.58 \\
            \hline
        \end{tabular}
    \end{table}
    ...
    \begin{figure}[h!]
        \centering
        % Assicurati di avere il file 'confusion_matrix_best.png' nella cartella immagini
        \includegraphics[width=0.6\textwidth]{images/graphs/bestConfusionMatrix}
        \caption{Matrice di Confusione del modello Logistic Regression (Pipeline 2). Sull'asse X le classi predette,
            sull'asse Y le classi reali.}
...
% -------------------------------------------------------------------------
    ...
    L'analisi comparata tra le due pipeline e i due algoritmi ci permette di trarre conclusioni non solo
    sulle performance numeriche, ma anche sulla natura stessa dei dati e su come i diversi algoritmi
    "ragionano" sulle cause della depressione.

    Una delle evidenze più interessanti emerge osservando quale feature viene considerata la "più importante"
    (valenza predittiva) dai due modelli nelle diverse configurazioni.

    \subsubsection{Convergenza sulla Pipeline 1: La forza del Feature Engineering}
    Nella \textbf{Pipeline 1}, entrambi gli algoritmi concordano all'unanimità. Sia il Decision Tree che la
    Logistic Regression identificano la feature ingegnerizzata \texttt{Stress\_Amplified}
    come il predittore dominante (con una \textit{Importance} del 75\%
    per il Tree e un \textit{Beta} di 1.47 per la Logistic).

    Questo dimostra che la nostra intuizione di combinare \textit{Pressione Accademica},
    \textit{Stress Finanziario} e \textit{Debito di Sonno} in un unico indicatore ha catturato con successo
    la complessità del fenomeno.

    \subsubsection{Divergenza sulla Pipeline 2: Logica "Globale" vs "Puntuale"}
    Nella \textbf{Pipeline 2}, dove le variabili sono state trattate singolarmente
    (senza feature engineering combinata), i due modelli mostrano una significativa differenza
    nell'identificazione del fattore principale:

    \begin{itemize}
        \item \textbf{Il Decision Tree} privilegia \texttt{Academic Pressure} (Importance: 0.48).
        \item \textbf{La Logistic Regression} privilegia \texttt{Financial Stress (Level 5)}
            ($\beta$: 2.27, Odds Ratio: 9.70).
    \end{itemize}

    Questa discrepanza riflette la diversa natura matematica degli algoritmi:
        Il \textbf{Decision Tree} trova nella \textit{Pressione Accademica}
            la variabile che meglio "divide" la popolazione in due
            grandi gruppi omogenei (miglior \textit{split}), mentre per

        la \textbf{Logistic Regression}, anche se avere
        \textit{Stress Finanziario a livello 5} è una condizione meno frequente rispetto ad
            avere pressione accademica, quando essa si verifica, la probabilità di depressione schizza
            alle stelle.
